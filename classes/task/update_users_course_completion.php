...
/**
 * Ad hoc task - Update users courses completion
 *
 * @package    block_completion_monitor
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace block_completion_monitor\task;
...
class update_users_course_completion extends \core\task\scheduled_task
{
    ...
    public function execute(): void
    {
        ...
        $countusercompletion = count($repository->get_last_users_completions($lastrows, true));
        ...
        for ($i = 0; $i < $iterations; $i++) {
            $userscompletionstoprocessed = $repository->get_last_users_completions($lastrows);
            $updates = [];
            ...
            if (!empty($updates)) {
                foreach ($updates as $update) {
                    $repository->set_user_course_completion(
                        $update['userid'],
                        $update['courseid'],
                        $update['completion'],
                    );
